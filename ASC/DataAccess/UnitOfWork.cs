...
namespace ASC.DataAccess
{
    public class UnitOfWork :IUnitOfWork
    {
        ...
        public IResponitory<T> Responitory<T>() where T : TableEntity
        {
            if (_respositories == null)
                _respositories = new Dictionary<string, object>();
             var type = typeof(T).Name;
              if (_respositories.ContainsKey(type)) return (IResponitory<T>)_respositories[type];
              ...
              return (IResponitory<T>)_respositories[type];
